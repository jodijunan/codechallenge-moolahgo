...
public interface UserApi {
    ...
    @ApiOperation(value = "Get user by referral", nickname = "getUserByReferral", notes = "", response = User.class, tags={ "user", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "successful operation", response = User.class),
        @ApiResponse(code = 400, message = "Invalid referral supplied"),
        @ApiResponse(code = 404, message = "User not found") })
    @RequestMapping(value = "/user/getByReferral/{referral}",
        produces = { "application/xml", "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<User> getUserByReferral(@ApiParam(value = "The referral that needs to be fetched.  ",required=true) @PathVariable("referral") String referral);
